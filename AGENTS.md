...
- **Before pushing / opening a PR, run `just ci`.** It runs the required CI checks locally — `actionlint` (workflow lint), `fmt`, `clippy` (both flavors), `test`+`coverage` (one instrumented build), `build` (minimal flavor; the full flavor is compiled by the coverage build), `deps-guard`, and `conformance` (the OCI distribution suite). Green `just ci` ⇒ green required CI. See [`README.md`](README.md) "Developing locally" for the full recipe table.
