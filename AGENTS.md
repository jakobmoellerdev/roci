...
- **100% line coverage is enforced.** `ci.yml`'s `test + coverage` job compiles the workspace **once** (instrumented) and runs the tests under `cargo llvm-cov nextest`, then enforces `--fail-under-lines 100` from that same build — tests and coverage share one compile rather than rebuilding. A PR that drops line coverage below 100% fails. New code MUST ship with tests that cover every line; inspect gaps with `just coverage-report`. The gate is strict on the Linux CI runner; `scripts/coverage.sh` is intentionally tolerant on non-Linux (e.g. macOS/APFS) where a couple of Unix-filesystem-specific edges cannot be exercised locally — those lines are covered on Linux CI, which remains authoritative. `main`'s entrypoint is covered by an integration test that runs the built binary (its coverage merges into the instrumented run).
