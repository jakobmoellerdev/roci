...
- **Before pushing / opening a PR, run `just ci`.** It runs the exact required CI checks locally — `actionlint` (workflow lint), `fmt`, `clippy` (both flavors), `test`, `build` (both flavors), `deps-guard`, and `coverage` (100% line coverage). Green `just ci` ⇒ green required CI. See [`README.md`](README.md) "Developing locally" for the full recipe table.
- **100% line coverage is enforced.** The `coverage` CI job runs `cargo llvm-cov --workspace --all-features --fail-under-lines 100`; a PR that drops line coverage below 100% fails. New code MUST ship with tests that cover every line. Inspect gaps with `just coverage-report`. The gate is strict on the Linux CI runner; `scripts/coverage.sh` is intentionally tolerant on non-Linux (e.g. macOS/APFS) where a couple of Unix-filesystem-specific edges cannot be exercised locally — those lines are covered on Linux CI, which remains authoritative.
- **Security & static analysis**: three additional gates run in CI — `actionlint` (a job in `ci.yml`, lints workflow YAML + shellchecks `run:` blocks), `zizmor.yml` (GitHub Actions security auditor, uploads SARIF to code scanning), and `codeql.yml` (CodeQL SAST for Rust, `build-mode: none`). Run `just lint-workflows` and `just zizmor` locally; CodeQL runs only on GitHub. When editing any workflow, keep it `actionlint`- and `zizmor`-clean (pin third-party actions by commit SHA, `persist-credentials: false` on checkout, no over-broad token permissions).
