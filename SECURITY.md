...
- **[interim status — Phase 2] Portable implementation of the above, with the same security guarantees.** Until the Linux fast paths land (Phase 5), the shipped code differs mechanically but preserves the invariants: upload staging is a **named `uploads/<id>` file** (not `O_TMPFILE`), the digest is still verified **before** the blob is renamed into the CAS, and **append/finish/abort on one session are serialized by a per-session lock** so no partially-verified or post-hash-appended bytes are ever promoted (closing the TOCTOU without the anonymous inode). Promotion is **fsync-then-atomic-rename** with a **containing-directory fsync** (not `linkat`), giving the same crash-safety — a torn write stays under `uploads/` and is discarded. Dedup/mount uses `hard_link`+`copy` (not reflink/`copy_file_range`); the shared-inode write-through risk stays bounded because blob fds are never opened for writing after promotion and CAS content is immutable. `O_NOFOLLOW`/`openat2 RESOLVE_BENEATH` are deferred — the charset-locked digest + `SafeComponent` path validation are the active traversal backstop. Manifest commit and the backref update are **not yet one atomic WAL record**: the backref index is a rebuildable cache and Phase 5 GC re-validates it from the layout before consuming it (a startup consistency check gates GC-enable), so the missing atomicity cannot cause live-content deletion.
