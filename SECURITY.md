...
| **HTTP Basic — LDAP** | Opt-in cargo feature `ldap` (roci-core `ldap`, roci-cli `ldap`; **not** in `full` — build with `--features full,ldap`); without it `auth.ldap` fails startup. Search-then-bind with a service account; `ldap_escape`'d filter; empty password rejected (anonymous-bind trap); TLS always verified; bounded by `timeout_secs`. Directory errors → warn + `401`. |
...
**Middleware order at runtime:** request span → early-data → global rate limit → authn → per-client rate limit → handler. The global rate limit (per-method buckets) protects the server before authentication. The per-client rate limit runs after authn, keyed by the authenticated principal identity (htpasswd/LDAP username, bearer `sub`, mTLS cert identity) when authenticated, else the TCP peer IP (socket address, **not** `X-Forwarded-For`; behind a proxy, all anonymous clients collapse onto the proxy IP — document this to operators). Keys are stored in a distinct enum (`ClientKey::Principal` / `ClientKey::Ip`) so a username can never collide with an IP. The per-client bucket map is LRU-bounded at `max_clients` (default 10 000); an evicted client restarts with a full bucket (accepted trade-off: a returning evicted client gets a fresh allowance, but the LRU cap bounds memory).
...
- **DoS bounds (see also §DoS in Storage boundary).** Separate **manifest size cap** (default ≤4 MiB, distinct from the blob cap) checked before JSON parse; bounded JSON recursion depth; per-session upload size cap; `n`/pagination parameters on tag-list and referrers capped server-side (never allocate `O(n)` from a client integer) and served by a storage-level seek, so per-request work is bounded by the page, never by the repo's tag or referrer count; wired read/write timeouts + global per-method rate limits + **per-client rate limits** (`http.rate_limit.per_client`; keyed by authenticated principal or peer IP, LRU-bounded at `max_clients`; see §Authorization middleware order). (CVE-2023-2253 / GHSA-hqxw-f8mx-cpmw catalog `n` OOM; GHSA-259w-8hf6-59bj referrers amplification.)
...
- **Fast-restart stamp is an integrity-relevant input.** The `.roci-fast-restart` stamp file seeds the presence filter, dedupe index, quota accounting, and GC candidate set — all security-relevant caches. It is protected by HMAC-SHA256 (when `storage.metadata.hmac_key_file` is configured) or CRC32C (otherwise). The stamp embeds a SHA-256 hash of the storage config and the metadata WAL generation + log length; any mismatch (config change, WAL mutation, binary upgrade, file tampering) rejects the stamp and forces a full CAS walk. The stamp is consumed (unlinked + dir-fsync) **before** validation, so a crash at any point after open forces a cold start, never a stale restore.
...
14. Every client-supplied count/size (`n`, manifest size, upload total) is bounded before allocation; timeouts, global per-method rate limits, and per-client rate limits (LRU-bounded) are wired.
