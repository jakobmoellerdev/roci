...
- **Implementation status — Phase 2.** The Linux fast paths above are **implemented** (`roci-storage`), each behind `#[cfg(target_os = "linux")]` with a portable fallback and covered on a real-Linux run (`just coverage-linux`): monolithic `put_blob` promotes via **`O_TMPFILE`+`linkat`** (anonymous inode, digest verified before it is namespace-visible; `EEXIST` = dedup); blob reads use **`openat2 RESOLVE_BENEATH|NO_SYMLINKS`** (`O_NOFOLLOW` elsewhere); dedup/mount tries **reflink (`FICLONE`) → `copy_file_range` → streaming copy**; presence/mount destinations require a **regular no-follow** entry. **Chunked upload staging is a named `uploads/<id>` file by design** — an `O_TMPFILE` inode cannot survive between the separate PATCH requests of a resumable upload — so its safety rests on: an **`O_NOFOLLOW`** append open (no symlink redirect), a **per-session lock** serializing append/finish/abort (no post-hash-appended bytes promoted), a **cap re-check inside the finalize lock**, digest verified **before** the atomic `rename`, and an `fsync` of the file, the CAS directory, and the layout marker/parents. A torn write stays under `uploads/` and is discarded. The single-record atomic WAL coupling of tag+referrers+backref remains deferred to Phase 5.
