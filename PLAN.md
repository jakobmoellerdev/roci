...
- [x] **Upload session manager:**
  ...
  - [x] **Streamed bodies** — every upload body (PATCH, finalizing PUT, monolithic POST) is written to the staging file frame-by-frame in 1 MiB batches on the blocking pool, never buffered whole (invariant 4); a failed/over-limit body truncates the staging file back so the session is unchanged. **Hash-on-write:** sha256 + CRC32C are computed as bytes land and kept with the session lock; finalize uses them when they cover the whole file, else (restart, sha512) re-hashes the staged file. **Few blocking hops:** the staging file is created lazily by the first write (an empty session is in-memory, expires like a stale upload, and does not survive restart), and finalize / manifest `PUT` run their filesystem steps as 2 / 1 blocking-pool hops (`registry.blocking.hops`).
...
- [x] Cross-repo **blob mount** (end-11) → `201` via the hard-link/reflink/copy path above (no read+write roundtrip), else falls through to a `202` session. *(Double-authz landed in Phase 6.)*
- [x] Crash-safety: `O_TMPFILE`+`linkat` (monolithic) or fsync-then-atomic-`rename` (chunked/mount), each with a containing-directory `fsync`, so an interrupted push never leaves a corrupt-but-named blob — a torn write stays under `uploads/` and is discarded. *(Single-record atomic WAL coupling of tag+referrers+backref landed in Phase 5 — atomic manifest record.)*
...
- [x] **Rate limiting**, **per-HTTP-method** token buckets (`http.rate_limit.per_method` + `default`; unlisted with no default → unlimited) → `429 TOOMANYREQUESTS` + `Retry-After`. Global per-method buckets (≤7, bounded memory); disabled → no layer at all. *(Per-client limiting landed after Phase 6: `http.rate_limit.per_client` buckets keyed by the authenticated principal, else the TCP peer IP, checked after authn; LRU-bounded at `max_clients` (default 10 000).)*
- [x] **Footprint knobs:** `mimalloc` global allocator (cargo feature `mimalloc`, in `full` + release/container builds; musl-friendly, no background threads); ext4 guidance: **do not** use `bigalloc` for manifest-dense stores — it raises the allocation unit to the cluster size (e.g. 64 KiB), wasting most of each small manifest/config file; keep 4 KiB blocks (`bigalloc` pays only for large-file-dominated volumes). *(`rkyv` mmap snapshot toggle landed in Phase 5 with the snapshot itself — `storage.metadata.snapshot`.)*
- [x] **OpenTelemetry telemetry pipeline** — `otel` builds export traces + metrics + logs over **OTLP** (gRPC or HTTP, batch processors) when `[telemetry.otlp]` is set; `ParentBased(TraceIdRatioBased(sample_ratio))` head sampler (default 1%); `service.name`/`service.version` resource; logs bridged via `opentelemetry-appender-tracing`. Root span carries semconv `http.request.method`/`url.path`/`http.route`/`http.response.status_code`; W3C `traceparent` extracted from requests (verified in Jaeger); child spans `meta.resolve`, `meta.append`, `blob.open`, `upload.session`, `digest.verify`. **Tail-based sampling is a collector concern** (OTel Collector `tail_sampling` processor keeping `status_code=ERROR` + latency policies), not in-process. *(Landed since: `authn.authorize` span (Phase 6); `gc.consistency_check`/`storage.maintenance{task}` sweep spans (Phase 5); `blob.stream`/`cas.link` child spans with the request span propagated into every storage blocking-pool hop. Still deferred: the peer-proxy hop with `roci-cluster`.)*
- [x] **Metrics via the OTel meter provider** — RED `http.server.request.duration` (seconds, semconv buckets; labels `endpoint`,`method`,`status_class`) + `registry.request.errors{error_code}`, defined once; **Prometheus scrape view** over the same `MeterProvider` at `telemetry.metrics.path` (default `/metrics`, never under `/v2`). Cardinality bounded by construction (endpoint is a route class, never a raw path). *(Storage instruments landed: GC/scrub/dedupe/quota (Phase 5), auth (Phase 6), MetadataStore `registry.meta.wal.appends`/`.wal.batch_size`/`.compaction`/`.snapshot` and upload `registry.upload.active`/`.bytes`/`.finalize{result}`. Cluster instruments land with Phase 8.)*
...
- [x] **Hardened Helm chart** (`charts/roci`) — single-replica StatefulSet, PSS restricted, per-workload NetworkPolicies, secure-by-default auth guard, optional HA S3 storage on RustFS (4-pod erasure set, one-pod-loss tolerance), SIGTERM graceful shutdown, bucket-init Job, digest-pinned dependency images, `values.schema.json` with `const`/enum guards. CI gate: `helm.yml` — lint + render guards (`just helm-lint`), k0s e2e matrix (`filesystem`, `s3-rustfs`) covering OCI conformance, NetworkPolicy enforcement, RustFS pod loss, graceful restart persistence.
...
- [x] **Index engine upgrade path** — feature-gated embedded B-tree KV: **redb** (`storage.metadata.engine = "redb"`, cargo feature `redb`, in `full`) behind the same trait, verified by a shared cross-engine behavioral suite; default stays append-log + maps. *(heed/LMDB vs redb bake-off settled: keep redb — RESEARCH §9.8.)*
- [x] **Fast restart** (`storage.fast_restart`, opt-in, FS backend) — on graceful shutdown an atomic, CRC32C- or HMAC-authenticated stamp (format + binary version, storage-config hash, metadata log position, presence/dedupe/quota/GC state) is written; startup consumes it before serving and, if every field validates, seeds from it and skips the CAS walk and GC consistency check; any mismatch or a crash (no stamp) falls back to the full walk. Out-of-band layout edits while stopped are not observed (as in zot).
...
- [x] Conformance suite in CI, run every phase — never regresses. *(`conformance.yml` — anonymous + htpasswd profiles — and `just ci`/the pre-commit hook run the full suite.)*
...
- [x] **Index-engine bake-off (Phase 1 deliverable, from RESEARCH §8.6):** benchmark **heed/LMDB vs redb** on roci's real access pattern (tag lookup, referrer range-scan, existence check) to settle the default; LMDB is predicted ~1.8–3× faster on reads / 35% smaller, redb is the pure-Rust/musl fallback. **Met:** `just bench-index` (standalone `bench/index-engines/`, outside the workspace) — LMDB 1.3–4× faster reads (≈6–7× at 8 threads) and 26–35% smaller, redb 3–6× faster writes; verdict **keep redb** (pure-Rust/musl, `forbid(unsafe_code)`), RESEARCH §9.8.
