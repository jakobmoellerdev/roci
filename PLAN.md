...
- [~] **Deduplication:** cross-path/-repo dedup via `std::fs::hard_link` (O(1), copy-free on one filesystem) with a `tokio::fs::copy` fallback (cross-device / no-hardlink); intra-path dedup is free (same digest → same CAS file). *(`copy_file_range`, `FICLONE` reflink, and `O_NOFOLLOW`/`openat2 RESOLVE_BENEATH` deferred — their Linux-only fast path is only reached on a cross-device mount, which the CI filesystem cannot exercise, so it cannot meet the 100%-Linux-coverage gate; the `SafeComponent` backstop already bars traversal.)*
