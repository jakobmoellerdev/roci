...
- [x] **Deduplication:** cross-path/-repo dedup via `std::fs::hard_link` (O(1), copy-free on one filesystem); on a hard-link failure (cross-device / no-hardlink) a **crash-atomic copy fallback** tries, in order, **reflink (`FICLONE`)** → **`copy_file_range`** → a `tokio` streaming copy, so a valid mount across devices/filesystems always completes (never a spurious `202`). On btrfs/XFS/bcachefs the reflink shares extents O(1) (CoW, independent deletion); `copy_file_range` keeps NFS/ext4 copies server- or kernel-side. The copy lands in a unique temp with `fsync` + `rename` (never a partial blob under the digest). Intra-path dedup is free (same digest → same CAS file). An existing mount destination is validated **no-follow** as a regular file before it counts as an idempotent `201`; a same-repo (`src == dest`) mount short-circuits. `#[cfg(target_os="linux")]` fast paths meet the 100% line gate on a real-Linux run via `just coverage-linux` (`Containerfile.coverage`). *(WAL group-commit tuning tracked in Phase 5.)*
