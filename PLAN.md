...
- [~] **Deduplication:** cross-path/-repo dedup via `std::fs::hard_link` (O(1), copy-free on one filesystem); on a hard-link failure (cross-device / no-hardlink) a **crash-atomic in-kernel copy fallback** — **`copy_file_range` on Linux** (btrfs/XFS share extents O(1) = reflink; ext4 in-kernel; NFS server-side — no userspace transit; RESEARCH §8.8) to a unique temp + `fsync` + `rename`, `tokio` streaming copy elsewhere. Intra-path dedup is free (same digest → same CAS file). The Linux fast path's coverage is exercised on a real-Linux gate via `just coverage-linux` (a `Containerfile.coverage` harness), so `#[cfg(target_os="linux")]` code meets the 100% line gate. *(Standalone `reflink-copy`/`FICLONE` across arbitrary filesystems and `O_TMPFILE`+`linkat`/`openat2 RESOLVE_BENEATH` remain Phase-5 items — the portable staging is now hardened with a per-session lock + no-follow guard + directory fsync, and `SafeComponent` bars traversal.)*
- [x] Cross-repo **blob mount** (end-11) → `201` via the portable hard-link/copy path above (no read+write roundtrip), else falls through to a `202` session. *(Double-authz in Phase 6.)*
...
- [ ] **Garbage collection — online, O(garbage), grace-period.** Live **backref multimap `blob_digest → {manifest_digests}`** in the MetadataStore → collect the instant a backref set empties AND `gcDelay` elapses AND not pinned by an in-flight upload; never offline; full mark-sweep only as cold-start/rebuild backstop. **Prerequisite (from Phase 2 review):** the backref index is populated *only* by the Phase-2 `PutBackrefs` log op, so a pre-existing/externally-built layout or an older metadata log has no edges for manifests written before the index existed. Before GC consumes backrefs it MUST **rebuild/validate them from the layout** (walk every manifest's config/layers/index-children/subject) at startup — a missing/torn edge otherwise reads as "unreferenced" and would delete live content. Startup consistency check gates GC-enable.
