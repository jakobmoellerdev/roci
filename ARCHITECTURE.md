...
**Where an embedded KV still earns its place — and how it stays optional.** The append-log + in-RAM maps model is ideal for the local-minimal single node: **zero external storage dependency, everything rebuildable from the layout.** It holds while the live metadata set fits in RAM. **[corrected from RESEARCH §9.4]** budget it honestly: ~**130 MB RSS per 1M tags** (32-byte keys + 32-byte digests + hashbrown overhead ≈ 130 B/entry), ~1.3 GB at 10M — *not* "tens of MB." The rkyv-mmap snapshot above is the RSS fix for memory-constrained nodes. When metadata truly outgrows RAM, or the compute-only cluster needs a *shared* store, the same `MetadataStore` **trait** is backed by the embedded B-tree (redb pure-Rust, or heed/LMDB — RESEARCH §8.6). The trait is the seam: `roci-minimal` ships the dependency-free append-log backend; the KV backend is a feature-gated upgrade, not a baseline requirement. Either way the layout stays the source of truth and the log/KV/snapshot are rebuildable caches.
