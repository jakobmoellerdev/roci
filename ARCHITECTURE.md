...
- **`roci-full`** — minimal + all extensions (search, signatures, scanning, sync, metrics UI, …) + the S3 storage backend (`s3`), the embedded redb metadata engine (`redb`), OpenTelemetry export (`otel`), and `mimalloc`. LDAP authentication (`ldap`) is opt-in and **not** part of `full` (`--features full,ldap`).
...
flowchart TB
    ...
    subgraph roci["roci process — single static binary"]
        ...
        subgraph storage["Storage subsystem (roci-storage)"]
            ...
            cas["local CAS: blobs/alg/hex · reflink dedup · O_TMPFILE+linkat · copy_file_range · sendfile"]
...
- **HTTP API layer.** Async server (axum/hyper), **HTTP/2 multiplexing + keep-alive** with HTTP/1.1 fallback, TLS 1.3 with optional kTLS and mTLS (`http.tls.client_auth`). Terminates TLS, parses/validates requests, applies read/write timeouts and rate limits, honors conditional requests (`ETag`/`If-None-Match` → `304`), routes to handlers. Every request is an OpenTelemetry span. **Middleware order at runtime** (outermost → innermost): connection-close (an HTTP/1 response sent before the request body was read to its end — e.g. a `401` on an unauthenticated upload — carries `Connection: close`: hyper closes that socket anyway, and a client that pooled it would send its retry into a dead connection) → request span → early-data (`425 Too Early` for non-idempotent `Early-Data: 1` per RFC 8470 §5.1) → global rate limit → authn → per-client rate limit → handler. The global rate limit protects the server before authentication; the per-client rate limit runs after authn so it can key on the authenticated principal (username or bearer `sub`), falling back to the TCP peer IP for anonymous clients. Roundtrip-efficiency design in §HTTP roundtrip efficiency.
- **Core (`roci-minimal`).** Distribution API handlers (all `end-1`..`end-13` endpoints), protocol types (`Digest`, `RepositoryName`, `Reference`, manifest models, the 14-code error enum), and the AuthN/AuthZ gate. **The core has zero dependency on any extension.** This is the seam that makes minimal builds real. The opt-in `ldap` cargo feature (not in `full`) adds LDAP authentication support.
...
- **Storage subsystem.** Two traits: **`Storage`** (blob CAS: reflink dedup, `O_TMPFILE`+linkat staging, `copy_file_range` mount, `sendfile`; local FS or remote object backend + dedupe cache) and **`MetadataStore`** (default append-log + in-RAM maps + WAL group-commit + rkyv snapshot; optional embedded B-tree KV), fronted by existence filters and a small-blob content cache. Detailed below.
...
- `http` — listen address/port, TLS (including `client_auth`, `client_ca`, `client_cert_sha256` for mTLS), read/write timeouts, rate limits (global per-method and per-client with LRU-bounded client map).
...
- **Local filesystem backend.** One or more root directories; content-addressable `blobs/<alg>/<hex>` store + `index.json`. Blob path is a pure function of digest → O(1) open. NFS/fuse mounts count as local. **[refined from RESEARCH §8.7]** the flat CAS is kept (correct for random-`open()`-by-digest; EXT4/XFS HTree handle tens of millions of files with stable reads). **[corrected — fanout rejected]** a git-style 2-level fanout (`blobs/<alg>/ab/cdef…`) was once planned above ~100K blobs per algorithm dir; it is **rejected**: the image-layout spec fixes the blob path as `blobs/<alg>/<encoded>` ([`image-layout.md`](spec/image-spec/image-layout.md) §Blobs), so a fanned-out tree would stop being a valid OCI layout that external tools can read (invariants 6 and 12). Its only payoff, `readdir` locality, doesn't matter here: GC marks from the in-memory backref index, and only the startup/scrub walks list a directory. A directory-scaling problem is handled by the filesystem (HTree/B-tree dirs), not by changing the layout. Venti-arena/packfile single-file stores are also **rejected** (they speed up sequential access at the cost of O(1) random access). `tar+zstd` layers (OCI v1.1) are **accepted and served natively**; no recompression at rest (recompression would change the digest — an OCI-contract violation).
...
- **Upgrade-backend engine choice: a single-file, embedded, B-tree-family KV with no background compaction threads.** When metadata outgrows RAM (very large multi-tenant deployments) or a shared store is needed, the trait is backed by a B-tree KV. roci's index workload is **read-heavy, modest-write**; the **RUM conjecture** (RESEARCH: RUM) → **LSM (RocksDB/sled/fjall) is the wrong fit** (2–5× worse reads, compaction threads fight the footprint budget). **[refined from RESEARCH §8.6]** **heed (LMDB)** is the stronger option where libc/C-FFI is acceptable — mmap single-level-store gives **1.8–3.0× faster random reads (3× at 16 threads) and 35% smaller on-disk** than redb (RESEARCH §8.6: redb-bench, LMDB-bench); **redb** (pure-Rust, no C) suits `no_std`/musl builds. Settled by the **Phase-1 benchmark deliverable**. Corroboration the *class* is right: zot's metadata backend is bbolt, a single-file B-tree.
  - **[implemented — Phase 5, measured — RESEARCH §9.8]** the upgrade engine ships as **redb** (`storage.metadata.engine = "redb"`, cargo feature `redb`, in `full`): the first-party bake-off (`just bench-index`, RESEARCH: RociIndexBench) confirms heed/LMDB is 1.3–4× faster on reads (up to 7× at 8 threads) and 17–26% smaller on disk, but redb is 3–6× faster on writes. **Verdict: redb is the correct choice** — pure Rust keeps the static-musl release and `forbid(unsafe_code)` story intact (heed/LMDB C FFI would break musl, deps-guard, and CodeQL scope), and the in-RAM `LogMetadataStore` is faster than *either* KV for the common sub-2M-ref case (§9.6). The `MetadataStore` trait seam is ready for heed if a deployment crosses >10M refs with a hard RAM cap. Layout (`<root>/roci-meta.redb`): composite-key tables so every query is a bounded range seek — tags `(repo, tag) → (digest, media_type)` with a `(repo, digest, tag)` reverse index; media types `(repo, digest)`; referrers `(repo, subject, referrer) → descriptor` plus `(repo, subject, artifactType, referrer)` for filtered pages and a `(repo, referrer, subject)` reverse index; backrefs as multimaps `(repo, blob) → {manifest}`; checksums `(repo, digest) → (crc32c, size)`.
...
- **Small-blob content cache [refined from RESEARCH §9.2].** Manifests/configs (1–50 KB) dominate request *count* (every pull reads them) but a trivial fraction of bytes. A bounded in-RAM `digest→bytes` **LRU cache** for blobs below `storage.small_blob_threshold` (**[implemented]** configurable, default 100 KB — the DB-vs-file crossover found independently by SQLite and Gray et al.; validated `0 < threshold ≤ 8 MiB`, `threshold ≤ cache_max_bytes` when the cache is enabled) serves them with **zero `open()`/`close()` syscall** → **35–80% latency cut** on the dominant request type (RESEARCH: SQLiteFasterFS, SQLiteInternBlob). The loose file still exists (OCI-conformant); the cache is a pure hot-path accelerator, miss → loose file. Large layer blobs are never cached — they go straight to `sendfile` (AIStore: +3× throughput, caching them would only pollute RAM; RESEARCH: AIStoreSendfile).
...
- **`index.json` is regenerated lazily/coalesced, not per-op.** After a tag mutation the in-memory tag map is authoritative immediately (the client's next pull sees it); the spec-visible `index.json` is rewritten by a **debounced, coalescing background task** (write-behind: many rapid tag ops collapse into one atomic replacement of `index.json`). **[implemented]** On Linux, writes use `O_TMPFILE` → `fsync` → `linkat` to a unique hidden temp → `renameat` onto `index.json` → dir `fsync`; the inode is namespace-invisible until `linkat`, so a crash during the write leaves no orphan. On non-Linux or when `O_TMPFILE` is unsupported, a named temp + `fsync` + `renameat` fallback is used. Startup reconciliation sweeps any stale `.index.json.*.tmp` orphans (regular files only, no-follow) so neither path accumulates leftovers. Readers that go through roci never wait on it; external tools reading the directory see an eventually-current, always-*valid* index. This turns the O(refs)-per-op wall into O(refs) amortized across a batch.
- **Log compaction is trivial and safe.** When `roci-meta.log` exceeds a threshold, a background pass writes a fresh log = the current in-memory state (one record per live key) via a fixed-name temp (`roci-meta.log.compact.tmp`) + `fsync` + `rename` + dir `fsync`, then drops the old log. A crash mid-compaction leaves the old log intact (atomic swap); the fixed temp name is harmlessly overwritten by the next compaction pass, so no orphans accumulate. Generation counters (`gen`) make replay idempotent and give the GC grace-period/epoch semantics for free.
- **O(1) cold start via rkyv mmap snapshot [refined from RESEARCH §9.4].** Log replay is negligible at ≤1M records (~20–40 ms NVMe) but grows to 300–500 ms at 10M / 15–20 s on HDD. A periodic **rkyv zero-copy snapshot** of the maps (written atomically via a fixed-name temp (`roci-meta.snapshot.snap.tmp`) + `fsync` + `rename` + dir `fsync`; the fixed name means a crash leaves at most one temp that is harmlessly overwritten by the next snapshot; replay only the post-snapshot log tail) makes cold start **O(1) — `mmap` + demand-paging, <5 ms regardless of size** — and cuts index RSS ~10× (demand-paged pages vs a heap map). rkyv access is a 1.09 ns pointer cast, no deserialize loop (RESEARCH: rkyv-bench, LMDB-SDC). Crates `rkyv` + `memmap2` (pure-Rust, no C). Enabled for large/HDD/embedded deployments; the `fastRestart` stamp doubles as the snapshot-fresh check. Never the *mutable* live format (the WAL is the source of durability).
...
- **[roci divergence] Fast restart.** zot's `fastRestart` skips the startup storage walk when a stamp matches binary+config identity. roci adopts this and pushes further: the startup walk is itself streaming and bounded so even a cold walk stays within the footprint budget. **[implemented — `storage.fast_restart`]** On graceful shutdown, an atomic stamp file (`.roci-fast-restart`) is written containing: format version, binary version, SHA-256 of the storage config, metadata WAL generation + log length, and the full seeded state (presence entries, dedupe index, per-repo quota bytes, upload sessions, GC candidates/roots/unsafe repos). Integrity is HMAC-SHA256 when `storage.metadata.hmac_key_file` is set, otherwise CRC32C. On startup with `fast_restart = true` the stamp is **consumed immediately** (removed + dir-fsync before any validation — so a crash forces a full walk); all fields are validated (format, binary version, config hash, metadata identity); on any mismatch/corruption/absence the server falls back to the existing full CAS walk with an info/warn log. Out-of-band edits while roci is stopped are not observed on fast restart (same as zot); a `fast_restart = false` restart always walks. S3 backend: not applicable (S3Storage does not perform a local CAS walk at startup).
...
The scratch image carries a CA bundle (data only, `SSL_CERT_FILE`): the S3 client (reqwest + `rustls-platform-verifier`) refuses to build when the system has no root certificates, even for the chart's plaintext in-cluster endpoint. The k0s `s3-rustfs` e2e caught this; images up to 0.2.0 lack the bundle.
...
- **MetadataStore:** `registry.meta.op.duration` (by `op`=`tag_set`/`resolve`/`referrers`/`backref`), `registry.wal.group_commit.batch_size` (histogram — proves group-commit is coalescing), `registry.wal.fsync.duration`, `registry.index.rss_bytes` (gauge — the memory-scaling invariant, alertable), `registry.cold_start.duration` (replay vs snapshot path). **[implemented — Phase 4 storage telemetry]** `registry.meta.wal.appends` (counter — each WAL record / redb txn commit), `registry.meta.wal.batch_size` (u64 histogram, boundaries 1–128 — group-commit coalescence proof), `registry.meta.compaction{result=ok|error}` (counter), `registry.meta.snapshot{result=ok|error}` (counter).
...
- **Upload sessions:** `registry.upload.active` (gauge), `registry.upload.duration`, `registry.upload.mount_hits` (cross-repo mount avoided a transfer). **[implemented — Phase 4 storage telemetry]** `registry.upload.active` (UpDownCounter / Prometheus gauge, +1 on begin, −1 on end/abort/discard), `registry.upload.bytes` (counter — bytes received across sessions), `registry.upload.finalize{result=ok|digest_mismatch|too_large|error}` (counter — upload completion outcomes). Recorded from both FS and S3 backends.
...
- **Process:** RSS/heap (allocator stats), FD count, open upload sessions, tokio runtime task/poll metrics, build info (`service.version`, features enabled). **[implemented]** `registry.blocking.hops{op}` counts every hand-off of filesystem work to the blocking pool (queue + two thread wake-ups, which outweigh a cached `stat`/`open`); storage operations batch their syscalls into as few hops as possible: `POST` does no filesystem work (the staging file is created by the session's first write — an empty session is in-memory only, expires with stale uploads and does not survive a restart), a body's final partial batch is written inside the next hop, an upload finalize is prepare → full 1 MiB batches → land, and a manifest `PUT` is one hop (`blob_exists` answers from the metadata record; the manifest commit's no-follow re-check stays authoritative). Measured push storm: 22.4 → 5.4 counted hops per image.
...
span: http.server (root)  attrs: http.route, oci.operation=end-3, oci.repository, client.addr
 ...
 ├─ blob.stream            attrs: bytes, range.start — **[implemented]** created in `BlobRead::into_stream`, propagated into every `spawn_blocking` read-chunk hop via the span handle
 └─ (push) upload.session ─ patch.chunk* → digest.verify → cas.link{mechanism=reflink|hardlink|copy|existing} ─ meta.append(group_commit) — **[implemented]** `cas.link` wraps `mount_promote_beneath` with bounded `mechanism` field
```
...
- **Cross-subsystem propagation:** **[implemented — Phase 4 storage telemetry]** the tracing context flows into storage-trait calls via `run_blocking` (which captures `Span::current()` and enters it on the blocking thread), `upload_body` write batches, and the metadata store. In cluster mode it also crosses the **peer proxy hop** so a forwarded request is one distributed trace across two instances. Background tasks (GC/scrub/sync sweeps) are their own root span trees, linked to the triggering config/schedule.
