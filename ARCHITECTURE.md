...
- **Deduplication [refined from RESEARCH §8.2].** Single physical copy of identical content across manifests. **Primary mechanism = reflink / CoW clone (`ioctl FICLONE`), not hard-link** — reflinks give identical O(1) space-sharing but with **independent deletion and no write-through-shared-inode hazard** (hard-links couple deletion into a smuggled refcount problem and risk mutating every repo sharing an inode). Supported on btrfs/XFS/APFS/ReFS; the `reflink-copy` crate **falls back to hard-link automatically** on ext4/NFS. CAS makes intra-path dedup free (same digest → same file); the dedup feature extends it *across* paths/repos. Sub-chunk dedup stays a **FUTURE `roci-ext-dedup`** (FastCDC — 10× faster than Rabin, `fastcdc` crate; RESEARCH §8.2: FICLONE-man, reflink-copy, FastCDC).
  - **[interim status — Phase 2]** The canonical design above (reflink-primary) is the target; the current implementation is the **portable subset**: cross-repo mount and cross-path dedup promote via `std::fs::hard_link` with a `std::fs::copy` fallback, and an existing destination is treated as an idempotent success (never re-copied — blobs are content-addressed). `FICLONE`/`copy_file_range` are deferred (their Linux-only path is only reached on a cross-device mount, which the coverage CI cannot exercise at the enforced 100% line gate). Safety limits of the interim path: hard-links couple deletion (mitigated by the online backref-GC grace period, Phase 5) and share an inode across repos (safe because CAS blobs are immutable/write-once — never modified in place). The reflink switch lands with Phase 5 dedup maturity.
...
12. `index.json`/`oci-layout`/`blobs/` stay spec-valid and externally consumable at all times, and the hot metadata index is a rebuildable derivative (append-only `roci-meta.log` + in-RAM maps, reconstructable from the layout). `index.json` is written atomically (`tmp`+`rename`) on each manifest mutation today; **write-behind coalescing** (many rapid tag ops → one rewrite) is a staged optimization (RESEARCH §9.3/§9.4), not yet active — it is an efficiency lever behind the `MetadataStore` seam, never a correctness requirement.
