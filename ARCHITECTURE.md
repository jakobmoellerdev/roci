...
12. `index.json`/`oci-layout`/`blobs/` stay spec-valid and externally consumable at all times; the hot metadata index is a rebuildable derivative and `index.json` is written write-behind (coalesced), never rewritten per tag op.
