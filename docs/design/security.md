...
Path-traversal-safe validation, a wire digest allowlist, bounded inputs (config-driven size / `n` caps, JSON depth), header-read + idle timeouts, global per-HTTP-method rate limits and per-client rate limits (`429 TOOMANYREQUESTS`, LRU-bounded client map keyed by principal or peer IP), TLS 0-RTT hardening, repository isolation (no cross-repo presence or content oracle), cross-repo mount double-authorization, and SSRF containment (no client-URL fetch; host-allowlisted, repo-gated redirects). On the storage side: storage quotas and a concurrent upload-session cap against exhaustion, GC that can never delete a blob a push is about to reference, a manifest and its derived links committed as one metadata record, scrub quarantine of blobs that no longer match their digest, and optional HMAC authentication of the metadata log and snapshot against a tampered-storage threat model. See the canonical [`SECURITY.md`](https://github.com/jakobmoellerdev/roci/blob/main/SECURITY.md) for the full boundary analysis, CVE mappings, and invariant list.
