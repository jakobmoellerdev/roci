...
cargo run -p roci-cli --features full     # + S3, redb, OpenTelemetry export, Prometheus /metrics, mimalloc
cargo run -p roci-cli --features full,ldap  # + opt-in LDAP authentication
```
